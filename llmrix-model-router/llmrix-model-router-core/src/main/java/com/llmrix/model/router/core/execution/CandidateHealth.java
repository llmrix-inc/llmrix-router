...
public final class CandidateHealth implements HealthState {
    private final AtomicInteger failures = new AtomicInteger();
    private final AtomicInteger inFlight = new AtomicInteger();
    private final AtomicLong cooldownUntilMillis = new AtomicLong();
    private volatile double latencyEwmaMillis;

    public boolean available(long nowMillis) { return nowMillis >= cooldownUntilMillis.get(); }
    public int inFlight() { return inFlight.get(); }
    public double latencyEwmaMillis() { return latencyEwmaMillis; }
    public void begin() { inFlight.incrementAndGet(); }
    public void cancel() { inFlight.decrementAndGet(); }
    ...
    @Override public HealthAttempt beginAttempt() { return tryBeginAttempt(null); }
            ...
            if (inFlight.compareAndSet(current, current + 1)) {
                return new HealthAttempt() {
                    ...
                    @Override public void cancel() {
                        if (settled.compareAndSet(false, true)) CandidateHealth.this.cancel();
                    }
                    @Override public void success(long durationNanos) {
                        if (settled.compareAndSet(false, true)) CandidateHealth.this.success(durationNanos);
                    }
                    @Override public boolean failure(long durationNanos, int threshold, Duration cooldown) {
                        return settled.compareAndSet(false, true)
                                && CandidateHealth.this.failure(durationNanos, threshold, cooldown);
                    }
                };
            }
        }
