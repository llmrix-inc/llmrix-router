...
public final class InMemoryBanditStateStore implements BanditStateStore {
    ...
    @Override
    public BanditArmStats arm(String namespace, String id) {
    ...
    @Override
    public void recordSelection(String namespace, String id) {
        MutableArm arm = state(namespace).computeIfAbsent(id, ignored -> new MutableArm());
        synchronized (arm) {
            arm.selections++;
        }
    ...
    @Override
    public void recordReward(String namespace, String id, double reward) {
        MutableArm arm = state(namespace).computeIfAbsent(id, ignored -> new MutableArm());
        synchronized (arm) {
            arm.observations++;
            arm.reward += reward;
    ...
    public Map<String, BanditArmStats> snapshot(String namespace) {
        ...
        state(namespace).forEach((id, arm) -> {
            synchronized (arm) {
                result.put(id, arm.snapshot());
            }
        });
