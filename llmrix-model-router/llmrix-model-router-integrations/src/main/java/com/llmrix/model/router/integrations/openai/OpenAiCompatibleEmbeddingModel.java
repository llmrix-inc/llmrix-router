...
public final class OpenAiCompatibleEmbeddingModel implements EmbeddingModel {
    ...
    public EmbeddingResponse embed(EmbeddingRequest request) {
        ...
        JsonNode root = transport.postJson("embeddings", payload);
        ...
        long inputTokens = usage.path("prompt_tokens").asLong(usage.path("input_tokens").asLong(-1));
        return new EmbeddingResponse(vectors, root.path("model").asText(modelName), new Usage(inputTokens, 0));
    }
