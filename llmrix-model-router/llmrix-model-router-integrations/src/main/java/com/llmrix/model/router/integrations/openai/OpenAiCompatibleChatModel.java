...
public final class OpenAiCompatibleChatModel implements ChatModel {
    ...
    public CompletionStage<ChatResponse> chatAsync(ChatRequest request) {
        ...
        transport.whenComplete((response, failure) -> {
            ...
            try {
                if (response.statusCode() < 200 || response.statusCode() >= 300) {
                    result.completeExceptionally(mapError(response.statusCode(), response.body()));
                } else {
                    ...
                    if (response.statusCode() < 200 || response.statusCode() >= 300) {
                        try (var lines = response.body()) {
                            if (control.terminate()) {
                                ...
                                publisher.closeExceptionally(mapError(response.statusCode(), body));
                            }
                        }
                        return;
                    }
    ...
    private ChatResponse parse(String body) {
        try {
            ...
            JsonNode usage = root.path("usage");
            Usage tokenUsage = usage.isMissingNode()
                    ? Usage.UNKNOWN
                    : new Usage(usage.path("prompt_tokens").asLong(-1), usage.path("completion_tokens").asLong(-1));
