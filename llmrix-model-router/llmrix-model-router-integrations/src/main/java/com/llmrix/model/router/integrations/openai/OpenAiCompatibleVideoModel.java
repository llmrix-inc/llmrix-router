...
public final class OpenAiCompatibleVideoModel implements VideoModel {
    ...
    public VideoResponse create(VideoRequest request) {
        if (routeModel != null && request.inputReference() == null) {
            ...
            return parse(transport.postJson("videos", body).toString());
        }
        ...
        return parse(responseBody(transport.postMultipart("videos", body)));
    }

    @Override
    public VideoResponse retrieve(VideoLookupRequest request) {
        return parse(responseBody(transport.get(withRoute("videos/" + path(request.videoId())))));
    }

    @Override
    public VideoContent content(VideoLookupRequest request) {
        OpenAiTransport.Response response = transport.get(withRoute("videos/" + path(request.videoId()) + "/content"));
    ...
    public VideoResponse delete(VideoLookupRequest request) {
        return parse(responseBody(transport.delete(withRoute("videos/" + path(request.videoId())))));
    }
    ...
    public VideoResponse remix(VideoRemixRequest request) {
        if (routeModel != null) {
            ...
            return parse(transport.postJson(
                    withRoute("videos/" + path(request.videoId()) + "/remix"), body).toString());
        }
        ...
        return parse(responseBody(transport.postMultipart(
                withRoute("videos/" + path(request.videoId()) + "/remix"), body)));
    }

    private String responseBody(OpenAiTransport.Response response) {
        try {
            return new String(response.body(), java.nio.charset.StandardCharsets.UTF_8);
        } catch (RuntimeException error) {
            throw new IllegalStateException("invalid video response", error);
        }
    }

    private VideoResponse parse(String body) {
        try {
            JsonNode root = transport.mapper().readTree(body);
            ...
            throw new com.llmrix.model.router.core.exception.ModelUnavailableException(
                    "invalid OpenAI-compatible video response", error);
        }
    }
