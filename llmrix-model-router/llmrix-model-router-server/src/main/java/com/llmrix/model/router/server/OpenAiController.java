...
import com.llmrix.model.router.core.api.StreamOptions;
...
public class OpenAiController {
    ...
    public Object chat(@RequestBody CompletionRequest request, HttpServletRequest servletRequest) {
        if (request.model() == null || request.model().isBlank()) throw new IllegalArgumentException("model is required");
        if (request.messages() == null || request.messages().isEmpty()) throw new IllegalArgumentException("messages are required");
        ...
        ChatRequest.Builder coreRequest = ChatRequest.builder()
                ...
                .generationOptions(new com.llmrix.model.router.core.api.GenerationOptions(
    ...
    private static List<Message> responsesInput(JsonNode input) {
        ...
        if (!input.isArray() || input.isEmpty()) throw new IllegalArgumentException("input must be text or a non-empty array");
    ...
    private static Message toCoreMessage(CompletionMessage message) {
        if (message.role() == null || message.role().isBlank()) throw new IllegalArgumentException("message role is required");
        ...
        if (!content.isArray() || content.isEmpty()) throw new IllegalArgumentException("message content must be text or a non-empty array");
        List<ContentPart> parts = new java.util.ArrayList<>();
        for (JsonNode part : content) {
    ...
    private static ResponseFormat toCoreResponseFormat(JsonNode value) {
        ...
        return switch (type) {
            ...
            case "json_schema" -> {
                ...
                if (!schema.isObject()) throw new IllegalArgumentException("json_schema response_format requires json_schema");
    ...
    public record CompletionMessage(
            String role,
            JsonNode content,
            @JsonProperty("tool_calls") List<CompletionToolCall> toolCalls,
            @JsonProperty("tool_call_id") String toolCallId) {
        public CompletionMessage(String role, String content) { this(role, TextNode.valueOf(content), List.of(), null); }
        public CompletionMessage(String role, JsonNode content) { this(role, content, List.of(), null); }
    }
    public record CompletionTool(String type, CompletionFunction function) { }
    public record CompletionFunction(String name, String description, Map<String, Object> parameters, Boolean strict) { }
    public record CompletionToolCall(String id, String type, CompletionFunctionCall function) { }
    public record CompletionFunctionCall(String name, String arguments) { }
    public record CompletionStreamOptions(@JsonProperty("include_usage") Boolean includeUsage) { }
